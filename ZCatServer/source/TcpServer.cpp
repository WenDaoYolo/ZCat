...
void TcpServer::Regist(int fd)
{
    ...
    error_code ec1=RegistModule(u1,oci[fd],user_db);
    int type=1;
...
void TcpServer::Login(int fd)
{
    ...
    error_code ec1=LoginModule(u1,oci[fd],isonline,user_db);
    int type=2;
...
void TcpServer::Apply(int fd)
{
    ...
    error_code ec1=ApplyModule(oci[fd].id,u1,user_db);
    int type=3;
...
{
    int size=GetApplyBuffer_len(oci[fd].admin,user_db);
    error_code ec1={7};
    ...
    if(size>0)
    {
        ...
        ec1=GetApplyBufferModule(blocks,oci[fd].admin,user_db);
...
{
    int size=GetFriendBuffer_len(oci[fd].admin,user_db);
    error_code ec1={7};
    ...
    if(size>0)
    {
        ...
        ec1=GetFriendBufferModule(blocks,oci[fd].admin,user_db);
...
{
    int size=GetUnicastMsgBuffer_len(oci[fd].id,user_db);
    error_code ec1={7};
    ...
    if(size>0)
    {
        ...
        ec1=GetUnicastMsgBufferModule(blocks,oci[fd].id,user_db);
...
void TcpServer::SendMulticastMsgBuffer(int fd)
{
    ...
    int size=GetMulticastMsgBuffer_len(gi.id,user_db);
    error_code ec1={7};
    ...
    if(size>0)
    {
        ...
        ec1=GetMulticastMsgBufferModule(blocks,gi.id,user_db);
...
{
    int size=GetGroupBuffer_len(oci[fd].id,user_db);
    error_code ec1={7};
    ...
    if(size>0)
    {
        ...
        ec1=GetGroupBufferModule(blocks,oci[fd].id,user_db);
...
{
    int size=GetGroupApplyBuffer_len(oci[fd].id,user_db);
    error_code ec1={7};
    ...
    if(size>0)
    {
        ...
        ec1=GetGroupApplyBufferModule(blocks,oci[fd].id,user_db);
...
void TcpServer::PermitApply(int fd)
{
    ...
    error_code ec=PermitApplyModule(u1.id,oci[fd].id,user_db);
    if(ec.value==0)
    {
        int tg_fd=FindTarget(u1.id);
        if(tg_fd)
...
void TcpServer::RefuseApply(int fd)
{
    ...
    RefuseApplyModule(u1.id,oci[fd].id,user_db);
}
...
void TcpServer::PermitGroupApply(int fd)
{
    ...
    error_code ec=PermitGroupApplyModule(ug,user_db);
    if(ec.value==0)
    {
...
void TcpServer::RefuseGroupApply(int fd)
{
    ...
    RefuseGroupApplyModule(ug,user_db);
}
...
void TcpServer::DeleteFriend(int fd)
{
    ...
    DeleteFriendModule(u1.id,oci[fd].id,user_db);
...
void TcpServer::MultiCast(int fd)
{
    ...
    MulticastMsgModule(mm1,user_db);
    int size=GetGroupMemberNumModule(mm1.group_id,user_db);
    int* g_ms_ptr=new int[size];
    GetGroupMemberIDModule(g_ms_ptr,mm1.group_id,user_db);
...
void TcpServer::CreateGroup(int fd)
{
    ...
    int type=14;
    error_code ec1=CreateGroupModule(gi1,user_db);
...
void TcpServer::GroupApply(int fd)
{
    ...
    int type=15;
    error_code ec=GroupApplyModule(oci[fd].id,gi1,user_db);
...
{   
    ...
    error_code ec1;
    int size=GetGroupMembers_len(gi1.id,user_db);//size at least be 2(manager and query people)
    ...
    ec1=ViewGroupMembersModule(blocks,gi1.id,user_db);
...
void TcpServer::RemoveGroupMember(int fd)
{
    ...
    RemoveGroupMemberModule(ug1,user_db);
...
void TcpServer::UpdatePasswd(int fd)
{
    ...
    error_code ec1=UpdatePasswdModule(u1,oci[fd],user_db);
    int type=23;
...
void TcpServer::AdminPanel()
{
    ...
    while(!stop)
    {
        ...
        if(!strcmp("help",buffer))
        {
            ...
            std::cout<<"'histroy                                display histroy update information"<<std::endl;
        ...
        if(!strcmp(buffer,"histroy"))
        {
            std::cout<<"2026-9-12:add database and port config file"<<std::endl;
            std::cout<<"2026-9-22:less database find times,improve performance"<<std::endl;
        }
    }
}
